...
*/
    ...
    // This method provides auto-completion for topology's names
	...
	try {
	    ...
	    if (id != null) {
		Topology topology = StormWrapper.getTopology(isLocal(), conf, id);
		return topology.getState().name(); //KILLED, PAUSED, RUNNING
	    } else {
		return null;
	    }
	} catch (NotAliveException e) {
